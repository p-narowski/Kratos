...
#pragma omp critical
    ...
    void ExplicitSolverStrategy::ComputeNodalArea()
    {
        ...
        { // each iteration refers to a different triangle or quadrilateral
    ...
    void ExplicitSolverStrategy::Check_MPI(bool &has_mpi)
    {
        ...
        if (r_modelpart_nodal_variables_list.Has(PARTITION_INDEX))
            has_mpi = true;
    }
...
#pragma omp parallel for schedule(dynamic, 50)
    ...
    void ExplicitSolverStrategy::SearchDEMOperations(ModelPart &r_model_part, bool has_mpi)
    {
        ...
        if (is_time_to_search_neighbours)
        {
            ...
            { // Just in case that some entities were marked as TO_ERASE without a bounding box (manual removal)
    ...
    void ExplicitSolverStrategy::SearchFEMOperations(ModelPart &r_model_part, bool has_mpi)
    {
        ...
        if (is_time_to_search_neighbours)
        { // for the moment it is always true, until all issues have been solved
...
#pragma omp for nowait
    ...
    void ExplicitSolverStrategy::BoundingBoxUtility(bool is_time_to_mark_and_remove)
    {
        ...
        if (r_process_info[DOMAIN_IS_PERIODIC])
        {
            ...
            if (is_time_to_mark_and_remove)
            { // in "periodic" condition, we should also could delete particles
                mpParticleCreatorDestructor->DestroyParticles<Cluster3D>(*mpCluster_model_part);
                mpParticleCreatorDestructor->DestroyParticles<SphericParticle>(r_model_part);
            }
        }
        ...
        if (fem_model_part.NumberOfSubModelParts())
        {
            ...
            for (ModelPart::SubModelPartsContainerType::iterator sub_model_part = fem_model_part.SubModelPartsBegin(); sub_model_part != fem_model_part.SubModelPartsEnd(); ++sub_model_part)
            {
                ...
                if (!r_process_info[IS_RESTARTED])
                {
                    ...
                    { // JIG: Backward compatibility, it should be removed in the future
...
#pragma omp parallel for
    ...
    void ExplicitSolverStrategy::SearchNeighbours()
    {
        ...
        // Defensive consistency check: the DEM particle list and the local element container
        // should represent the same search domain here.
...
#pragma omp parallel for schedule(dynamic, 100)
        for (int i = 0; i < number_of_particles; i++)
        {
            ...
            SphericParticle *p_particle = mListOfSphericParticles[i];

            KRATOS_ERROR_IF(p_particle == nullptr)
                << "Null particle pointer in mListOfSphericParticles at index " << i << std::endl;

            KRATOS_ERROR_IF(i >= static_cast<int>(this->GetResults().size()))
                << "SearchNeighbours results size mismatch: i = " << i
                << ", GetResults().size() = " << this->GetResults().size() << std::endl;

            p_particle->mNeighbourElements.clear();

            if (p_particle->Is(DEMFlags::POLYHEDRON_SKIN))
            {
            ...
            for (SpatialSearch::ResultElementsContainerType::iterator neighbour_it = this->GetResults()[i].begin();
                 neighbour_it != this->GetResults()[i].end(); ++neighbour_it)
            {
                ...
                if (!p_spheric_neighbour_particle)
                    continue;
                if (p_spheric_neighbour_particle == p_particle)
                    continue;

                if (p_particle->Is(DEMFlags::BELONGS_TO_A_CLUSTER) &&
                    (p_particle->GetClusterId() == p_spheric_neighbour_particle->GetClusterId()))
                {
                    continue;
                }

                p_particle->mNeighbourElements.push_back(p_spheric_neighbour_particle);

                std::vector<SphericParticle *> &neighbours_of_this_neighbour_for_this_thread =
                    thread_maps_of_connectivities[OpenMPUtils::ThisThread()][p_spheric_neighbour_particle];

                neighbours_of_this_neighbour_for_this_thread.push_back(p_particle);
            }
...
#pragma omp parallel for schedule(dynamic, 100)
        for (int i = 0; i < number_of_particles; i++)
        {
            ...
            KRATOS_ERROR_IF(p_particle == nullptr)
                << "Null particle pointer in symmetry loop at index " << i << std::endl;

            auto &current_neighbours = p_particle->mNeighbourElements;
            std::vector<SphericParticle *> neighbours_to_add;

            for (size_t k = 0; k < thread_maps_of_connectivities.size(); k++)
            {
                ConnectivitiesMap::iterator it = thread_maps_of_connectivities[k].find(p_particle);
            ...
            {
                SphericParticle *p_candidate = neighbours_to_add[l];
                if (!p_candidate)
                    continue;
                if (p_candidate == p_particle)
                    continue;

                bool found = false;
                for (size_t m = 0; m < current_neighbours.size(); m++)
                {
                    if (p_candidate == current_neighbours[m])
                    {
                        found = true;
                        break;
                    }
                ...
                if (!found)
                {
                    current_neighbours.push_back(p_candidate);
                }
            }
        }
...
#pragma omp parallel for schedule(dynamic, 100)
        for (int i = 0; i < number_of_particles; i++)
        {
            ...
            KRATOS_ERROR_IF(p_particle == nullptr)
                << "Null particle pointer in final validation at index " << i << std::endl;

            for (size_t j = 0; j < p_particle->mNeighbourElements.size(); ++j)
            {
                KRATOS_ERROR_IF(p_particle->mNeighbourElements[j] == nullptr)
                    << "Null neighbour pointer found for particle Id "
                    << p_particle->Id()
                    << " at neighbour index " << j << std::endl;
            }
        }
        KRATOS_CATCH("")
    }
...
#pragma omp for
            for (int i = 0; i < number_of_particles; i++)
            {
                ...
                for (unsigned int j = 0; j < neighbors_size; j++)
                {
                    ...
                    if (neighbour_element == NULL)
                        continue; // The initial neighbor was deleted at some point in time!!
                    if (mListOfSphericParticles[i]->Id() > neighbour_element->Id())
                        continue;
            ...
            for (int i = 0; i < number_of_particles; i++)
            {
                ...
                for (unsigned int j = 0; j < neighbors_size; j++)
                {
                    ...
                    for (unsigned int k = 0; k < neighbour_element->mNeighbourElements.size(); k++)
                    {
                        // ATTENTION: Ghost nodes do not have mContinuumIniNeighbourElements in general, so this bond will remain as NULL!!
                        // In all functions using mBondElements we must check that this bond is not used.
                        if (neighbour_element->mNeighbourElements[k] == NULL)
                            continue; // The initial neighbor was deleted at some point in time!!
