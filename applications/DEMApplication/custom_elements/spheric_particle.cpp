...
namespace Kratos
{
    ...
    void SphericParticle::Initialize(const ProcessInfo &r_process_info)
    {
        ...
        if (this->IsNot(BLOCKED))
            node.GetSolutionStepValue(PARTICLE_MATERIAL) = GetParticleMaterial();
        ...
        if (node.GetDof(VELOCITY_Y).IsFixed())
        {
        ...
        if (node.GetDof(VELOCITY_Z).IsFixed())
        {
        ...
        if (node.GetDof(ANGULAR_VELOCITY_X).IsFixed())
        {
        ...
        if (node.GetDof(ANGULAR_VELOCITY_Y).IsFixed())
        {
        ...
        if (node.GetDof(ANGULAR_VELOCITY_Z).IsFixed())
        {
                                                             ...
                                                             std::vector<array_1d<double, 3>> &temp_neighbour_elastic_contact_forces)
    {
        ...
        for (unsigned int i = 0; i < new_size; i++)
        {
            ...
            { // This is required by the continuum sphere which reorders the neighbors
                                                                 ...
                                                                 array_1d<double, 3> &r_contact_force)
    {
        ...
        for (int i = 0; data_buffer.SetNextNeighbourOrExit(i); ++i)
        {
            ...
            if (CalculateRelativePositionsOrSkipContact(data_buffer))
            {
                ...
                { // TODO: we should avoid calling a processinfo for each neighbour. We can put it once per time step in the buffer??
                                                                      ...
                                                                      const ProcessInfo &r_process_info)
    {
        ...
        for (unsigned int i = 0; i < rNeighbours.size(); i++)
        {
            ...
            DEM_SET_COMPONENTS_TO_ZERO_3x3(data_buffer.mLocalCoordSystem)
                DEM_SET_COMPONENTS_TO_ZERO_3x3(data_buffer.mOldLocalCoordSystem)
                    array_1d<double, 3>
                        wall_delta_disp_at_contact_point = ZeroVector(3);
            array_1d<double, 3> wall_velocity_at_contact_point = ZeroVector(3);
            bool sliding = false;
                                      ...
                                      DEMWall *wall)
    {
        ...
        if (WallBrinellHardness)
            InverseOfWallBrinellHardness = 1.0 / WallBrinellHardness;
        else
            KRATOS_ERROR << "Brinell hardness cannot be zero!";
        ...
        if (sliding)
            volume_wear = WallSeverityOfWear * InverseOfWallBrinellHardness * std::abs(LocalElasticContactForce) * sqrt(Sliding_0 * Sliding_0 + Sliding_1 * Sliding_1);
        ...
        if (element_area)
        {
            ...
            KRATOS_ERROR << "A wall element with zero area was found!";
        ...
        if ((shape_functions_coefs[0] >= 0.0) && (shape_functions_coefs[1] >= 0.0) && (shape_functions_coefs[2] >= 0.0))
        { // Which means the ball is inside this element
    ...
    void SphericParticle::CorrectRepresentativeVolume(double &rRepresentative_Volume /*, bool& is_smaller_than_sphere*/)
    {
        ...
        { // In case it gets 0.0 (discontinuum). Also sometimes the error can be too big. This puts some bound to the error for continuum.
    ...
    void SphericParticle::MemberDeclarationFirstStep(const ProcessInfo &r_process_info)
    {
        ...
        if (r_process_info[ROTATION_OPTION])
            this->Set(DEMFlags::HAS_ROTATION, true);
        else
            this->Set(DEMFlags::HAS_ROTATION, false);

        if (r_process_info[ROLLING_FRICTION_OPTION])
            this->Set(DEMFlags::HAS_ROLLING_FRICTION, true);
        else
            this->Set(DEMFlags::HAS_ROLLING_FRICTION, false);

        if (r_process_info[GLOBAL_DAMPING_OPTION])
            this->Set(DEMFlags::HAS_GLOBAL_DAMPING, true);
        else
            this->Set(DEMFlags::HAS_GLOBAL_DAMPING, false);

        if (r_process_info[COMPUTE_STRESS_TENSOR_OPTION])
            this->Set(DEMFlags::HAS_STRESS_TENSOR, true);
        else
            this->Set(DEMFlags::HAS_STRESS_TENSOR, false);

        if (r_process_info[PRINT_STRESS_TENSOR_OPTION])
            this->Set(DEMFlags::PRINT_STRESS_TENSOR, true);
        else
            this->Set(DEMFlags::PRINT_STRESS_TENSOR, false);
    ...
    double SphericParticle::GetInitialDeltaWithFEM(int index)
    { // only available in continuum_particle
        return 0.0;
    }
    ...
    void SphericParticle::RotateOldContactForces(const double OldLocalCoordSystem[3][3], const double LocalCoordSystem[3][3], array_1d<double, 3> &mNeighbourElasticContactForces)
    {
        ...
        v1[0] = OldLocalCoordSystem[2][0];
        v1[1] = OldLocalCoordSystem[2][1];
        v1[2] = OldLocalCoordSystem[2][2];
        v2[0] = LocalCoordSystem[2][0];
        v2[1] = LocalCoordSystem[2][1];
        v2[2] = LocalCoordSystem[2][2];
    ...
    void SphericParticle::CalculateOnContactElements(size_t i, double LocalContactForce[3], double GlobalContactForce[3])
    {
        ...
        if (!mBondElements.size())
            return; // we skip this function if the vector of bonds hasn't been filled yet.
        ParticleContactElement *bond = mBondElements[i];
        if (bond == NULL)
            return; // This bond was never created (happens in some MPI cases, see CreateContactElements() in explicit_solve_continumm.h)
    ...
    void SphericParticle::SetInteractionRadius(const double radius, const int radius_index)
    {
        mRadius = radius;
    ...
    void SphericParticle::SetMass(double real_mass)
    {
        mRealMass = real_mass;
        GetGeometry()[0].FastGetSolutionStepValue(NODAL_MASS) = real_mass;
    }
