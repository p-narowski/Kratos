...
namespace Kratos
{
    ...
    namespace
    {
            ...
            double rLocalCoordSystem[3][3])
        {
            ...
            tangent_1[0] =
                normal[1] * tangent_0[2] - normal[2] * tangent_0[1];

            tangent_1[1] =
                normal[2] * tangent_0[0] - normal[0] * tangent_0[2];

            tangent_1[2] =
                normal[0] * tangent_0[1] - normal[1] * tangent_0[0];
                 ...
                 ++local_direction)
            {
                     ...
                     ++global_direction)
                {
                    rLocalVector[local_direction] +=
                        rLocalCoordSystem[local_direction][global_direction] * rGlobalVector[global_direction];
                }
            }
        }
                 ...
                 ++local_direction)
            {
                     ...
                     ++global_direction)
                {
                    rGlobalVector[global_direction] +=
                        rLocalCoordSystem[local_direction][global_direction] * rLocalVector[local_direction];
                }
            }
        }
        ...
        {
            return rContactProperties.Has(DPD_SMOOTHING_LENGTH) && rContactProperties.Has(DPD_CUTOFF_RADIUS) && rContactProperties.Has(DPD_CONSERVATIVE_COEFF) && rContactProperties.Has(DPD_DISSIPATIVE_COEFF_NORMAL) && rContactProperties.Has(DPD_DISSIPATIVE_COEFF_TANGENTIAL);
        }
        ...
        const array_1d<double, 3> &rGravity)
    {
        ...
        // Mark this typed element and its node as suspended.
        if (!this->Is(DEMFlags::IS_SUSPENDED_PARTICLE))
        {
            this->Set(DEMFlags::IS_SUSPENDED_PARTICLE, true);
        }

        auto &r_node = this->GetGeometry()[0];

        if (!r_node.Is(DEMFlags::IS_SUSPENDED_PARTICLE))
        {
            r_node.Set(DEMFlags::IS_SUSPENDED_PARTICLE, true);
        }
